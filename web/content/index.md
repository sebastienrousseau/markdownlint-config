...
screenshot_alt: "@sebastienrousseau/markdownlint-config architecture and rule enforcement."
footer_note: "@sebastienrousseau/markdownlint-config is part of the @sebastienrousseau/config suite. Published under Apache-2.0 OR MIT."
copyright: "© 2026 Sebastien Rousseau. Licensed under Apache-2.0 OR MIT."
translation_key: "home"
title: "@sebastienrousseau/markdownlint-config — Shareable markdownlint configuration rules for consistent, accessible Markdown documentation."
description: "Shareable markdownlint configuration rules for consistent, accessible Markdown documentation."
keywords: "markdownlint-config, markdownlint, markdown, linter, documentation style, commonmark, developer tools, sebastien rousseau, standard"
hero_tag: "v0.0.3"
hero_tagline: "Consistent, accessible Markdown documentation linting rules"
headline: "@sebastienrousseau/markdownlint-config"
lead: "Shareable markdownlint configuration rules for consistent, accessible Markdown documentation."
cta_primary: "Install @sebastienrousseau/markdownlint-config"
...
snippet_caption: "Install @sebastienrousseau/markdownlint-config"
snippet_code: "npm install --save-dev @sebastienrousseau/markdownlint-config"
...
## What is @sebastienrousseau/markdownlint-config?

**@sebastienrousseau/markdownlint-config** is an enterprise-grade, shareable configuration for [markdownlint](https://github.com/sebastienrousseau/markdownlint-config)—the static analysis linter and style checker for Markdown and CommonMark files. Designed to deliver vetted, deterministic defaults, this package eliminates manual setup boilerplate and establishes rigorous code quality standards across modern software development pipelines.

By adopting this configuration, engineering teams ensure unified consistency across local development, continuous integration, and automated deployments. The package adheres strictly to the OpenSSF Best Practices badge program (ID: 14512) and achieves top-tier security ratings via Scorecard audits.
...
## Why Developers Choose @sebastienrousseau/markdownlint-config

Modern JavaScript and TypeScript repositories frequently suffer from configuration drift, fragmented standards, and maintenance overhead. **@sebastienrousseau/markdownlint-config** directly solves these challenges:
...
Integrating `@sebastienrousseau/markdownlint-config` into any new or existing repository requires only three simple steps.
...
### Step 2: Configure `.markdownlint.json`

Create or update your `.markdownlint.json` file at the root of your project to extend the shared configuration:

```json
{
  "extends": "@sebastienrousseau/markdownlint-config"
}
```

Learn how to customize specific rules and override settings in the [Usage Guide](usage/).

### Step 3: Run markdownlint in Your Workflow

Execute markdownlint directly or configure standard quality scripts in your `package.json`:

```bash
npx markdownlint-cli "**/*.md"
```

Scan documentation files for header hierarchy errors, trailing whitespace, and broken table formatting.
...
### What Markdown standards are enforced?
It enforces heading level increment standards, list indentation rules, proper code block fences, and prevents raw unescaped HTML to maximize documentation readability.

### Can I override individual rules in my project?
Yes. Every configuration allows local overrides directly within your `.markdownlint.json` file or inline settings. Detailed override recipes are documented in the [Usage Guide](usage/).

### Is @sebastienrousseau/markdownlint-config safe for production CI/CD?
